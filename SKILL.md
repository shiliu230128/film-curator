...
- 单次点击、跳过或评分不能直接改写长期禁区。偏好结论必须保留证据来源。评分记在 `work_rating`（对外叫「我的评分」）；`fit_rating`（当时适配度）可选，只在用户表达「片子好但当时看不进去」时才记，用来避免把时机问题学成内容禁区。
...
| 看完、弃看 | 确认版本；记录我的评分、可选的当时适配度和短评或弃看原因；更新历史、推荐反馈与有限的画像信号。 |
